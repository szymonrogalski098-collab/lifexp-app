...
(function () {
  ...
  function startCoins() {
    ...
    let basketX, basketVX, targetX, items, floats, bokeh;
    let score, combo, state, elapsed, spawnAcc, spawnEvery, diedAt, isRecord;
    let shakeT, flashA, hitStopT, basketSquash;

    // Particle pool — game-visual-polish sekcja 3: unikamy push/filter co
    // klatkę (GC-stutter przy dużym combo). Stała pula, obiekty oznaczane
    // jako `alive`, nigdy nie tworzone/usuwane w locie.
    const PARTICLE_POOL_SIZE = 120;
    const particlePool = [];
    for (let i = 0; i < PARTICLE_POOL_SIZE; i++) particlePool.push({ alive: false, x: 0, y: 0, vx: 0, vy: 0, life: 0, max: 0.8, r: 0, color: '#fff' });
    ...
    function reset() {
      ...
      shakeT = 0; flashA = 0; hitStopT = 0; basketSquash = 0;
    }
    ...
    function burst(x, y, color, n) {
      ...
      for (let i = 0; i < particlePool.length && spawned < n; i++) {
        ...
        p.life = 0.5 + Math.random() * 0.3; p.max = p.life; p.r = 2 + Math.random() * 2; p.color = color;
        spawned++;
      }
      // Pula wyczerpana → najstarsze particle po prostu nie odświeżą się w tej
      // klatce (recycle-oldest efektywnie następuje przy kolejnym wygaśnięciu).
    }

    // Hit-stop (game-visual-polish sekcja 2): krótkie zamrożenie klatki w
    // momencie trafienia — czytelniejsze niż sam particle/dźwięk. ~70ms.
    ...
    function catchItem(it) {
      ...
      triggerHitStop(it.value >= 5 ? 90 : 55); // droższa moneta = odrobinę mocniejszy stop
      basketSquash = 1; // wyzwala squash&stretch koszyka w draw()
    }
    ...
    function step(dt) {
      // Hit-stop: podczas aktywnego okna zamrażamy symulację przez zerowanie
      // efektywnego dt — draw() nadal się wykonuje, więc klatka "zamiera"
      // zamiast migać. Odliczane realnym dt, nie zamrożonym.
      ...
      for (const p of particlePool) {
        if (!p.alive) continue;
        p.vy += 240 * dt; p.x += p.vx * dt; p.y += p.vy * dt; p.life -= dt;
      ...
      if (basketSquash > 0) basketSquash = Math.max(0, basketSquash - dt * 6); // ~165ms zanik
    ...
    function drawBasket() {
      ...
      // Squash&stretch (game-visual-polish sekcja 2): przy złapaniu koszyk
      // krótko spłaszcza się szeroko, potem wraca — czytelne potwierdzenie
      // trafienia niezależnie od particli/tekstu.
      if (!reduce && basketSquash > 0) {
        const s = basketSquash;
        ctx.scale(1 + s * 0.16, 1 - s * 0.14);
      }
    ...
    function draw() {
      ...
      drawBasket();
      for (const p of particlePool) { if (!p.alive) continue; ctx.globalAlpha = Math.max(0, p.life / p.max); ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill(); }
      ctx.globalAlpha = 1;
      ctx.textAlign = 'center';
